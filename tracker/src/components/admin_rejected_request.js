...
function Admin_p_r(props){
    ...
    useEffect(()=>{
        (async ()=>{
           try{
                    let responserequest = await axios.get("/api/admin/get-all-rejected-request");
    ...
    return (
        <div className="row">
            <div className="col-sm-6 col-sm-offset-3 rm-padding-lr">
                <h1 className="text-center"> Rejected Requests <span>{<NavLink to="/admin" className="btn btn-primary">Back to Admin</NavLink>}</span></h1>
                {request.length > 0 ?
